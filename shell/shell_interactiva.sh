...
chatgpt() {
    # echo -n "🤖 Escribe tu pregunta para ChatGPT: "
    # read pregunta

    # if [[ -z "$pregunta" ]]; then
    #     echo "No ingresaste ninguna pregunta. Inténtalo de nuevo."
    #     return
    # fi

    API_KEY="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"
    # Solicitar al usuario que introduzca un prompt
    read -p "Introduce tu mensaje para ChatGPT: " MENSAJE

    # Verificar que el mensaje no esté vacío
    if [[ -z "$MENSAJE" ]]; then
        echo "Error: No puedes dejar el mensaje en blanco."
        exit 1
    fi

    # Hacer la solicitud a la API y almacenar la respuesta en una variable
    RESPUESTA_JSON=$(curl -s -X POST "https://api.openai.com/v1/chat/completions" \
    ...
    if echo "$RESPUESTA_JSON" | jq -e '.error' > /dev/null; then
        ERROR=$(echo "$RESPUESTA_JSON" | jq -r '.error.message')
        echo "Error en la API: $ERROR"
        exit 1
    fi

    # Extraer el contenido de la respuesta usando jq
    RESPUESTA=$(echo "$RESPUESTA_JSON" | jq -r '.choices[0].message.content')

    # Verificar si la respuesta es null
    if [[ "$RESPUESTA" == "null" || -z "$RESPUESTA" ]]; then
        echo "Error: No se recibió una respuesta válida de ChatGPT."
        exit 1
    fi

    # Mostrar la respuesta
    echo "Respuesta de ChatGPT: $RESPUESTA"

    # También puedes almacenar la respuesta en una variable para usarla más tarde
    RESPUESTA_ALMACENADA="$RESPUESTA"
    echo "Respuesta almacenada: $RESPUESTA_ALMACENADA"
}
